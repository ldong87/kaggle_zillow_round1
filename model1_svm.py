...
def model_run(tr_x, tr_y, tr_x17, tr_y17, quarter,clf):
    ...
    pid = pid.to_frame().assign(f_svm=pred)
    return pid
...
# year 2016
#%%
clf = SVR(kernel='rbf', degree=6, gamma=0.001, coef0=0.0, tol=0.0001, C=0.01, 
...
#%%
with open('featQ23_svm.pkl','wb') as f:
...
#%%
with open('featAll16_svm.pkl','wb') as f:
    pk.dump([feat_10, feat_11, feat_12], f, protocol=pk.HIGHEST_PROTOCOL)
    
write_sub([feat_10.f_svm.as_matrix(),feat_11.f_svm.as_matrix(),feat_12.f_svm.as_matrix(),
           feat_10.f_svm.as_matrix(),feat_11.f_svm.as_matrix(),feat_12.f_svm.as_matrix()])
#%%    
...
with open('featQ4567_svm.pkl','wb') as f:
...
#%%
with open('featAll17_svm.pkl','wb') as f:
    pk.dump([feat_22, feat_23, feat_24], f, protocol=pk.HIGHEST_PROTOCOL)
    
write_sub([feat_10.f_svm.as_matrix(),feat_11.f_svm.as_matrix(),feat_12.f_svm.as_matrix(),
           feat_22.f_svm.as_matrix(),feat_23.f_svm.as_matrix(),feat_24.f_svm.as_matrix()])