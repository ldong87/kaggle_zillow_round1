...
def model_run(train, test, model):
...
def model_pred(metadata, quarter, params):
    ...
    pred = model.predict(te_x, thread_count=8)
    pid = pid.to_frame().assign(f_catb=pred)
...
model = CatBoostRegressor(
    iterations=2000, learning_rate=0.005, rsm=0.75,
...
featQ3L2 = model_run(metaQ2, metaQ3, model)
...
feat2_10 = model_pred([metaQ2, metaQ3, feat_10], 3, model)
feat2_11 = model_pred([metaQ2, metaQ3, feat_11], 3, model)
feat2_12 = model_pred([metaQ2, metaQ3, feat_12], 3, model)
with open('featAll16L2_xgb.pkl','wb') as f:
    pk.dump([feat2_10, feat2_11, feat2_12], f, protocol=pk.HIGHEST_PROTOCOL)
#%%
write_sub([feat2_10.f_catb.as_matrix(),feat2_11.f_catb.as_matrix(),feat2_12.f_catb.as_matrix(),
           feat2_10.f_catb.as_matrix(),feat2_11.f_catb.as_matrix(),feat2_12.f_catb.as_matrix()])    
#%%
# year 2017
...
model = CatBoostRegressor(
    iterations=1000, learning_rate=0.015, rsm=1,
    depth=4, l2_leaf_reg=6, bagging_temperature=1,
    loss_function='MAE',
    eval_metric='MAE',
    random_seed=0,
    thread_count=8,
...
featQ7L2 = model_run(metaQ23456, metaQ7, model)
metaQ7L2 = featQ7L2.join(train_y.logerror)

with open('catb_metaQ7L2.pkl','wb') as f:
    pk.dump(metaQ7L2, f, protocol=pk.HIGHEST_PROTOCOL)
    
#with open('featQ4567L2_xgb.pkl','wb') as f:
...
model = CatBoostRegressor(
    iterations=1000, learning_rate=0.01, rsm=1,
    depth=6, l2_leaf_reg=6, bagging_temperature=1,
    loss_function='MAE',
    eval_metric='MAE',
    random_seed=0,
...
with open('featAll17L2_xgb.pkl','wb') as f:
    pk.dump([feat2_22, feat2_23, feat2_24], f, protocol=pk.HIGHEST_PROTOCOL)
#%%
write_sub([feat2_10.f_catb.as_matrix(),feat2_11.f_catb.as_matrix(),feat2_12.f_catb.as_matrix(),
           feat2_22.f_catb.as_matrix(),feat2_23.f_catb.as_matrix(),feat2_24.f_catb.as_matrix()]) 